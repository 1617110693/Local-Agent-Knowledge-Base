...
- **显示模式**：卡片、网格 或 紧凑列表 — 工具栏一键切换
- **排序**：按名称 (升/降)、日期 (升/降) 或默认（注册表顺序）
- **置顶**常用知识库 — 置顶项始终排在最前
...
- **暗色/亮色/系统** 主题切换 (☀️/🌙/🖥️)
- **中/英文** 界面
- **侧边栏**：KB 列表（可滚动）+ 设置固定底部，置顶图标
- **KB 工作区**：统计信息、文档管理、网格/列表视图切换、搜索入口
- **文档预览**：支持 LaTeX 公式，长描述可点击展开阅读
- **搜索界面**：混合/向量/关键词模式切换，显示 rerank 模型
- **设置面板**：连接测试、MCP 配置生成器、**ZIP 导入/导出**、**一键清空**
